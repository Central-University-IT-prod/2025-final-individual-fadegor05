...
class DeleteCampaignUseCaseProtocol(Protocol):
    async def __call__(self, advertiser_id: UUID, campaign_id: UUID) -> None: ...
...
class DeleteCampaignUseCaseImpl:
    ...
    async def __call__(self, advertiser_id: UUID, campaign_id: UUID) -> None:
        ...
        await self.campaign_repository.hide(campaign.id)


class DeleteCampaignUseCaseProvider(Provider):
    ...
    def provide_dependency(
        ...
        return DeleteCampaignUseCaseImpl(campaign_repository, advertiser_repository)
